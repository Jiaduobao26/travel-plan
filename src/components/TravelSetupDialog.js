...
const TravelSetupDialog = ({ open, onClose, title, onConfirm, destination = { name: '', place_id: '' } }) => {
    ...
    const designTrip = async () => {
        ...
        const payload = {
            ...
            budget: budget || '', // "Upscale"
            preferences: {
                travelStyle: preferences?.travelStyle.map(o => o.value) || [],
                transportation: preferences?.transportation.map(o => o.value) || [],
                dietaryRestrictions: dietaryRestrictions.map(o => o.value) || [],
                mustSeePlaces: mustSee || '',   // 字符串
            },
        };

        try {
            setSubmitting(true);
            const res = await getAttractions(payload);
            console.log(res)
            // 把结果回传给父级（如果传了 onConfirm）
                ...
                <Box sx={{ flex: 1, pr: 2 }}>
                    <TabContext value={tabValue}>
                        ...
                        <TabPanel value="3" keepMounted={true}>
                            ...
                            />
                            <TravelBudget onPrev={() => setTabValue('2')} onNext={() => setTabValue('4')} budget={budget} onChange={(e) => setBudget(e)} />
                        </TabPanel>
